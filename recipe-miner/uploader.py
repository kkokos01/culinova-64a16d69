    ...
    def upload_recipe(self, recipe_data: Dict, qa_data: Dict, batch_id: str) -> Optional[str]:
        ...
        try:
            ...
            if qa_data['status'] == 'FLAG':
                description = f"⚠️ QA Flagged: {qa_data['reason']}\n\n{description}"
            ...
            # Upload using atomic transaction
            result = supabase.rpc("create_recipe_with_ingredients", {
                "recipe_data": recipe_payload,
                "ingredients_data": ingredients,
                "steps_data": steps
            }).execute()
            
            recipe_id = result.data
            return recipe_id
